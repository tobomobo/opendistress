...
|---|---|---|
| 1. Foreground TEST | Garmin v1 sender plus phone-configured direct Grafana Cloud IRM and optional Pushover TEST, fixed alert content, post-acceptance personal GPS drill, explicit outcomes | Private-beta install, physical Garmin ping/GPS, receiver drill, and 100-attempt matrix |
...
| 6. Location | Post-trigger encrypted LIVE snapshot/fresh/cadence plus bounded multi-provider direct GPS drill, persisted foreground cadence, quality/movement/battery backoff, signed LIVE incident-status stop | Real indoor/outdoor direct and LIVE GPS, lifecycle, battery, restart, and resolution-stop tests; mocks do not pass |
...
| 8. More transports | Relay Pushover/ntfy plus bounded direct Garmin-to-Grafana/Pushover TEST | Grafana ACK correlation, direct-provider duplicate matrix, and ntfy device/deduplication matrix; SMS/voice remain unclaimed without companion/SIM/provider hardware |
| 9. Native watches | Independent Wear OS Kotlin and watchOS Swift clients with durable queues, v2 crypto, location, feedback, and hosted builds; one Android setup app adds encrypted Wear OS provisioning, digest-bound Garmin TEST provisioning, one-shot post-acceptance phone-location assist, direct TEST parity, and Pixel-Watch-sized emulator coverage | Paired-emulator Data Layer and Connect IQ Mobile SDK drills, provider trial, Android background/location matrix, and physical watches |
